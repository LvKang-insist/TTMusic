...
public class SpreadView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < spreadRadius.size(); i++) {
            ...
            int widht = spreadRadius.get(i);
            //绘制一个 spread 圆
            canvas.drawCircle(centerX, centerY, widht + radius, spreadPaint);
            //每画完一个圆进行更新当前圆的透明度和半径
            if (alpha > 0 && widht < 300) {
                alpha = alpha - distance > 0 ? alpha - distance : 1;
                alphas.set(i, alpha);
                spreadRadius.set(i, widht + distance);
            }
            Log.e("for:", "onDraw: " + i);
        }
        // 重置
